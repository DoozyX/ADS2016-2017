...
class Graph<E> {
	...
	@SuppressWarnings("unchecked")
	Path dijkstraPath(int from) {

		/* Minimalna cena do sekoj od teminjata */
		Path path = new Path();
		path.distance = new float[this.num_nodes];
		path.path = new String[this.num_nodes];

		for (int i = 0; i < this.num_nodes; i++) {
			path.path[i] = "" + adjList[from].getInfo();
		}
		...
		for (int i = 1; i < this.num_nodes; i++) {
			...
			while (it.hasNext()) {
				...
				if (!finalno[pom.node.getIndex()]) {
					/* ako ne e presmetana cena za temeto */
					if (path.distance[pom.node.getIndex()] <= 0) {
						path.distance[pom.node.getIndex()] = path.distance[from]
								+ pom.weight;
						path.path[pom.node.getIndex()] = path.path[from] + " " + pom.node.getInfo();
					}
					/* inaku, ako e pronajdena poniska */
					else if (path.distance[from] + pom.weight < path.distance[pom.node
							.getIndex()]) {
						path.distance[pom.node.getIndex()] = path.distance[from]
								+ pom.weight;
						path.path[pom.node.getIndex()] = path.path[from] + " " + pom.node.getInfo();
			...
			/* proveri gi site teminja */
			for (int j = 0; j < this.num_nodes; j++) {
				if (!finalno[j] && path.distance[j] != -1) { /*ako cenata ne e  konecna*/
					if (!minit) { /* ako ne e inicijaliziran minimumot */
						minc = path.distance[k = j]; /* proglasi go ova e minimum */
						minit = true; /* oznaci deka min e inicijaliziran */
					}
					/* proveri dali e pronajdeno teme so pomala cena */
					else if (minc > path.distance[j] && path.distance[j] > 0)
						minc = path.distance[k = j];
				}
			}
